...
static int
IsAlphaNum(char c) /* is this a valid char for a pathname or filename? */
{
  if (c >= 'A' && c <= 'Z')
    return 1;
  if (c >= 'a' && c <= 'z')
...
static int CopyWord(char *token, struct CommandData *data, int *state) {
  ...
  case COMMAND:
    ...
    *state = ARG;
    return 1;
  case ARG:
    data->TheCommands[data->numcommands]
        .args[data->TheCommands[data->numcommands].numargs] = strdup(token);
